...
class Utf8
{
    ...
    private static $cp1252 = array('','','','','','','','','','','','','','','','','','','','','','','','','','','');
    private static $utf8   = array('€','‚','ƒ','„','…','†','‡','ˆ','‰','Š','‹','Œ','Ž','‘','’','“','”','•','–','—','˜','™','š','›','œ','ž','Ÿ');
    ...
    public function __construct($string, $turkish = false)
    {
        if (!preg_match('//u', $s)) {
            throw new \InvalidArgumentException("Invalid UTF-8 string: ".$string);
        }

        $this->string = (string) $string;
        $this->turkish = (bool) $this->turkish;
    }
     ...
     * @return static
     */
    public function toAscii($substChr = '?')
    {
        $s = $this->string;

        if (preg_match("/[\x80-\xFF]/", $s)) {
            static $translitExtra;
            if (!$translitExtra) {
                $translitExtra = static::getData('translit_extra');
            }
     ...
     * @return static
     */
    public function toCaseFold($full = true)
    {
        $s = $this->string;
     ...
     * @return static
     */
    public function toNatFold()
    {
        $result = clone $this;
        $result->string = preg_replace('/\p{Mn}+/u', '', n::normalize($result->string, n::NFD));
    ...
    {
        return static::filter(json_decode($this->string, $assoc, $depth, $options));
    }
